...
public class PromptProcessor {
    ...
    public Flux<String> retrieveAndGenerateContextual(String message) {
        ...
        // Build information section from the grouped documents
        ...
        try {
            return aiStream.concatWith(
                    // Append references for the user
                    Flux.just(objectMapper.writeValueAsString(additionalInfo))
            );
            ...
            tokenFlux.subscribe(
                    token -> {
                        ...
                        if (buffer.length() >= chunkSize) {
                            // Use {{{ and }}} to ensure streaming doesn't use trim() to necessary whitespace
